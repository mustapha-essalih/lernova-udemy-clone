...
export function Expired() {
	...
	const onSubmit = async ({ email }: { email: string }) => {
		...
		try {
			await axios.post(
				`http://localhost:8080/api/v1/auth/resend-email-verification`,
				{ email }
			);
			...
			<StatusContainer>
				...
				{!open && (
					<button
						...
						className="bg-orange-500 py-2 px-8 rounded-full text-white font-medium text-lg cursor-pointer"
					>
						Request new one
					</button>
				)}
				{!emailSent && open && (
					...
					>
						<input
							...
							className="border border-gray-300 rounded-full py-2 px-4 w-full focus:outline-none focus:ring-2 focus:ring-orange-500"
						/>
						{errors.email && (
						...
						<button
							...
							className="bg-orange-500 w-[105px] h-[44px] rounded-full text-white font-medium text-lg cursor-pointer flex items-center justify-center"
						>
							{isSubmitting ? (
								<CircularProgress size={24} thickness={4.9} color="inherit" />
							) : (
								"Send"
							)}
						</button>
