...
public class GeneraleService {
    ...
    public UserResponse getUserInfos(String username, String jwt) {
        ...
        if (student != null)
            return new UserResponse(student.getId(), student.getFirst_name(), student.getLast_name(),
                    student.getEmail(), student.getUsername(), student.getRole().name(), jwt);

        Instructors instructor = instructorsRepository.findByUsername(username).orElse(null);

        if (instructor != null)
            return new UserResponse(instructor.getId(), instructor.getFirst_name(), instructor.getLast_name(),
                    instructor.getEmail(), instructor.getUsername(), instructor.getRole().name(), jwt);
